    ...
    for layer in model.layers:
        ...
        if len(weights) > 0:
            ...
            if len(filter_shape) > 2:  # For convolutional layers
                ...
                filter_weight = np.transpose(filter_weight, (2, 3, 1, 0))   #Height Weight Batch Channel
...
def yolo_net_out_to_car_boxes(net_out, threshold=0.2, sqrt=1.8, C=20, B=2, S=7):  #If not given setting it values given in original paper
    class_num = 6 #In original trained weights , vehicle class number is 6
    boxes = []
    ...
    for grid in range(SS):
        for b in range(B):
            ...
            bx.y = (cords[grid, b, 1] + grid // S) / S   # // menas floor division
    ...
    for i in range(len(boxes)):
        ...
        for j in range(i + 1, len(boxes)):
            boxj = boxes[j]
            if box_iou(boxi, boxj) >= .5:   # Parameter tuning
                boxes[j].prob = 0.
